...
<h1 class="d-flex justify-content-center mb-5">Список заявок</h1>
<div class="container d-flex gap-4 p-0">
    <div class="accordion" id="completed-tasks-accordion">
        <h4 class="mx-3 my-3">В ожидании</h4>
        <?php foreach ($newTasks as $task): ?>
            <?php renderTask($task, 'К выполнению'); ?>
        <?php endforeach; ?>
    </div>
    <div class="accordion" id="tasks-accordion">
        <h4 class="mx-3 my-3">К выполннению</h4>
        <?php foreach ($tasks as $task): ?>
            <?php renderTask($task, 'Выполнено'); ?>
        <?php endforeach; ?>
    </div>
    <div class="accordion" id="completed-tasks-accordion">
        <h4 class="mx-3 my-3">Выполненно</h4>
        <?php foreach ($completedTasks as $task): ?>
            <?php renderTask($task, 'Архив'); ?>
        <?php endforeach; ?>
    </div>
</div>
<?php 
...
?>
<?php
function renderTask($task, $currentStatus)
{
?>
    <div class="accordion-item" style="width: 400px;">
            ...
            <h2 style="width: 400px;">
                <button  class="accordion-button collapsed " type="button" data-bs-toggle="collapse" data-bs-target="#task-collapse-<?php echo $task['id']; ?>" aria-expanded="false" aria-controls="task-collapse-<?php echo $task['id']; ?>">
                    <div class="d-flex justify-content-between w-100">
                        <span class="ml-1 text-dark" style="overflow: hidden; text-overflow: ellipsis; width: 150px">
                            <strong><?php echo htmlspecialchars($task['client']['full_name'] ?? ''); ?> </strong>
                        </span>
        ...
        <div id="task-collapse-<?php echo $task['id']; ?>" class="accordion-collapse collapse" aria-labelledby="task-<?php echo $task['id']; ?>" data-bs-parent="#tasks-accordion">
            <div class="accordion-body" style="width: 400px; white-space: normal;">
            <p class="row m-0">
                <div class="mb-2">
            ...
            <p class="row m-0">
                ...
                <div>
                    <strong><i class="fa-solid fa-city"></i> Название города:</strong> 
                    <?php echo htmlspecialchars($task['city_info'][0]['city_name'] ?? '');?>
                </div>
                <div>
                    <strong><i class="fa-solid fa-city"></i> Область:</strong> 
                    <?php echo htmlspecialchars($task['city_info'][0]['area_name'] ?? ''); ?>
                </div>
                <div>
                ...
                <div class="d-flex">
                    ...
                    <form action="/crm/todo/tasks/update-status/<?php echo $task['id']; ?>" method="post" class="me-2">
                        ...
                        <button type="submit" class="btn btn-success"><?php echo $currentStatus ?></button>
                    </form>
                </div>
            </div>
        </div>     
